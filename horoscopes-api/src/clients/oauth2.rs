...
pub fn gen_google_oauth_client() -> BasicClient {
    let google_oauth_client_id = env::var(GOOGLE_OAUTH_CLIENT_ID_ENV_KEY)
        .expect(&format!(
            "Failed to fetch env: {}.",
            GOOGLE_OAUTH_CLIENT_ID_ENV_KEY
        ));
    let google_oauth_client_secret =
        env::var(GOOGLE_OAUTH_CLIENT_SECRET_ENV_KEY).expect(&format!(
            "Failed to fetch env: {}.",
    ...
    .set_redirect_url(
        RedirectUrl::new(GOOGLE_OAUTH_REDIRECT_URL.to_string()).unwrap(),
    );
