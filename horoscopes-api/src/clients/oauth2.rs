...
const GOOGLE_OAUTH_CLIENT_SECRET_ENV_KEY: &str = "GOOGLE_OAUTH_CLIENT_SECRET";

const GOOGLE_OAUTH_TOKEN_URL: &str = "https://accounts.google.com/o/oauth2/token";
const GOOGLE_OAUTH_AUTH_URL: &str = "https://accounts.google.com/o/oauth2/auth";
const GOOGLE_OAUTH_REDIRECT_URL: &str = "http://localhost:3030/oauth2/callback";

pub fn gen_google_oauth_client() -> BasicClient {
    let google_oauth_client_id = env::var(GOOGLE_OAUTH_CLIENT_ID_ENV_KEY).expect(&format!(
        "Failed to fetch env: {}.",
        GOOGLE_OAUTH_CLIENT_ID_ENV_KEY
    ));
    let google_oauth_client_secret = env::var(GOOGLE_OAUTH_CLIENT_SECRET_ENV_KEY).expect(&format!(
        "Failed to fetch env: {}.",
    ...
    .set_redirect_url(RedirectUrl::new(GOOGLE_OAUTH_REDIRECT_URL.to_string()).unwrap());
