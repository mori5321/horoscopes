...
pub fn filter(
       ...
       + Clone {
    let deps = update_todo_usecase::Deps::new(Arc::new(
...
mod tests {
    ...
    fn init_usecase() -> (
        ...
        let deps =
            update_todo_usecase::Deps::new(todo_repository.clone());
        let usecase =
            update_todo_usecase::UpdateTodoUsecase::new(deps);
    ...
    async fn handler_updates_todo_properly() {
        ...
        let req_body = RequestBody { todo: update_todo };
        let rep = handler("ulid-0001".to_string(), req_body, usecase)
            .await
            .unwrap();
        ...
        let new_todo = todo_repository
            .find(todo::ID::from_str("ulid-0001"))
            .unwrap();

        assert_eq!(
            new_todo.title().value(),
            "What kind of fool I am".to_string()
        );
    ...
    async fn handler_returns_not_found_error_when_invalid_id_passed()
    {
        ...
        if let Err(rejection) =
            handler("invalid_id".to_string(), req_body, usecase).await
        {
            let err = rejection
